...
# Quickshell owns video playback; this script only prepares files and colors.
VIDEO_THUMBNAIL=""
is_video() {
    local extension="${1##*.}"
    case "${extension,,}" in mp4|webm|mkv|avi|mov|m4v) return 0 ;; *) return 1 ;; esac
}
...
switch() {
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if [[ ! -f "$imgpath" ]]; then
            echo "Wallpaper file not found: $imgpath" >&2
            return 1
        fi
        check_and_prompt_upscale "$imgpath" &

        if is_video "$imgpath"; then
            [[ -n "$VIDEO_THUMBNAIL" ]] || return 1
            if [[ -z "$noswitch_flag" ]]; then
                jq --arg path "$imgpath" --arg thumbnail "$VIDEO_THUMBNAIL" '
                    .background.video //= {} |
                    (if (.background.wallpaperPath | test("\\.(mp4|webm|mkv|avi|mov|m4v)$"; "i")) then .
                     else .background.video.lastImagePath = .background.wallpaperPath end) |
            ...
            matugen_args=(image "$VIDEO_THUMBNAIL")
            generate_colors_material_args=(--path "$VIDEO_THUMBNAIL")
        else
...
main() {
    ...
    local palette_image="$imgpath"
    if is_video "$imgpath"; then
        local prepared
        prepared=$(python3 "$SCRIPT_DIR/video-thumbnail.py" "$imgpath") || return 1
        imgpath=$(jq -r '.path' <<< "$prepared")
        VIDEO_THUMBNAIL=$(jq -r '.thumbnail' <<< "$prepared")
        palette_image="$VIDEO_THUMBNAIL"
    fi

    # If type_flag is 'auto', detect scheme type from image (after imgpath is set)
    if [[ "$type_flag" == "auto" ]]; then
        if [[ -n "$imgpath" && -f "$imgpath" ]]; then
            detected_type="$(detect_scheme_type_from_image "$palette_image")"
